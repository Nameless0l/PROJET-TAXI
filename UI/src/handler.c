    ...
    if(signum == OC_HAS_SCHEDULED)
    {
        ...
        if (can_read_shm)
        {
            ...
            for (int i = 0; i < nb_pairs; ++i)
            {
                ...
                Point *client_dest = get_quarter_pos(datas->client_dest);
                Point *client_start = get_quarter_pos(datas->client_start);
                Point *client_center = get_quarter_pos(datas->client_start);
                ...
                Point *bike_start = get_quarter_pos(datas->bike_start);
                Point *bike_center = get_quarter_pos(datas->bike_start);
                bike->side = _SIDE;
                bike->center = bike_center;
                bike->start = bike_start;

                push_front(_global_bc_list, datas->bike_pid, datas->client_pid, bike, client);
